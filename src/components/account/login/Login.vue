    ...
    export default {
        ...
        data() {
         ...
         methods: {
             ...
             login() {
                 // post请求的第二个参数是发送的数据, 这里直接把data里的表单对象(this.ruleForm2)传过去
                //  this.$http.post(this.$api.login, this.ruleForm2).then(() => {
                //      this.$alert('登陆成功');
                //首先调接口，发送登录请求(1---this.$api.login调用api.js文件登录接口，2----这里直接把data里的表单对象(this.ruleForm2)传过去)
                             ...
                             callback: () => {
                               // 保存用户姓名
                                localStorage.setItem('user_name', res.data.message.user_name);       
                                 // 使用了路由插件之后, 组件实例就拥有了该对象, 对象有一个push方法, 登陆成功可以进行路由跳转
                                 let nextPage = this.$route.query.next || '/goods/list';
                                 // 登陆成功后, 跳转到用户未登陆前要访问的页面 现在登录成功后直接跳转到/goods/list
             ...
             // 登陆按钮方法
             // this.$refs用来获取页面中的元素或组件(然后再调方法), 这里通过它拿到form表单组件, 调用validate方法, 给所有表单进行校验
             submitForm(formName) {
